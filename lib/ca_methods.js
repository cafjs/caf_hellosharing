...
var getRandomFunction = function(value, inverse) {
    return (inverse ?
            'function(x) { return x - this.get("rand") - ' + value + ';}' :
            'function(x) { return x + this.get("rand") + ' + value + ';}');
};
...
exports.methods = {
    ...
    '__ca_pulse__' : function(cb) {
        ...
        if (isAdmin(this)) {
            var x = Math.random();
            $$.master.setFun('f', getRandomFunction(x));
            $$.master.setFun('fInv', getRandomFunction(x, true));
        ...
        if ($$.slave && (typeof $$.slave.get('rand') === 'number')) {
            ...
            async.timesSeries(NITER, function (n, cb0) {
                ...
                if (!err) {
                    self.getState(null, function(error, data) {
                        if (error) {
                            cb(error);
                        } else {
    ...
    'getState' : function(version, cb) {
        ...
        var map = ($$.slave ? this.$.sharing.pullUpdate('slave', version) :
                   null);
